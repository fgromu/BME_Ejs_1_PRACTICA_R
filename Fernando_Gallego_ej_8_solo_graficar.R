...
winDialog(type="ok", message=paste("Numero de preguntas sera ",n_preguntas_c))     
j = which(n_preguntas_c == lista_empresas_id)


Numero_empresas_revisa = 1
media_todos_los_cierres = 0
stop_profit = 0
stop_loss = 0
dinero_max = 0
volumen_gloval = 0

j = which(lista_empresas_id==nombre_a_plotear_querido)
...
media_todos_los_cierres = (sum(aux_Ibex_data$close)) / (length(aux_Ibex_data$close))
volumen_gloval = (sum(aux_Ibex_data$vol)) / (length(aux_Ibex_data$vol))
dinero_max = media_todos_los_cierres * (volumen_gloval*0.005)

stop_profit = quantile(aux_Ibex_data$high-aux_Ibex_data$open,c(0.3))
stop_loss = quantile(aux_Ibex_data$open-aux_Ibex_data$low,c(0.8))
...
for (i in 1: length(aux_Ibex_data$open)){
  ...
  if(length(aux_Ibex_data$open) >= 30 ) {
    ...
    if (aux_Ibex_data[i,9] >= 0.75){
      contador = contador+1
      n_acciones_compro = dinero_max/aux_Ibex_data$open[i]
      invertido = round(dinero_max,3)
      gasto_compra = round(invertido*0.0003,3)

      if(aux_Ibex_data$low[i] < (aux_Ibex_data$open[i] - stop_loss)){
        precio_salir = (aux_Ibex_data$open[i] - stop_loss) * n_acciones_compro
      }else if(aux_Ibex_data$high[i] > (aux_Ibex_data$open[i] + stop_profit)){
        precio_salir = (aux_Ibex_data$open[i] + stop_profit) * n_acciones_compro
      }else{
